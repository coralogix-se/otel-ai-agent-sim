...
Resource ``service.name=github-copilot``. Span tags aligned with Coralogix AI Center Copilot CLI dashboard
(cxai-demo HAR): ``otel.scope.name=github.copilot``, ``enduser.pseudo.id`` (roster email by default; optional opaque),
``github.copilot.cost``,
``gen_ai.usage.*`` / ``gen_ai.usage.cache_read.input_tokens`` on ``invoke_agent``.
See https://code.visualstudio.com/docs/copilot/guides/monitoring-agents
"""
...
def emit_copilot_cli_session(
    ...
    *,
    ...
    tracer = st.sim_cli.github_copilot.get_tracer(scope_nm, ver)
    cx_app = os.environ.get("COPILOT_CX_APPLICATION_NAME", "github-copilot")
    cx_sub = os.environ.get("COPILOT_CX_SUBSYSTEM_NAME", "copilot-cli-sessions")
